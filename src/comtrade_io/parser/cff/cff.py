...
class CffFile:
    ...
    def to_configure(self) -> Configure | None:
        ...
        try:
            return CfgFileParser.from_str(self.cfg_text)
        except Exception as e:
            logger.error(f"解析 CFG 配置失败: {e}")
            return None

    def to_data_content(self, cfg: Configure) -> pd.DataFrame | None:
        """将 DAT 部分转换为 pd.DataFrame（不生成临时文件）

        参数:
            cfg: Configure 配置对象

        返回:
            pd.DataFrame | None: 数据内容，解析失败返回None
        """
        ...
        try:
            if cfg.data_type.value == "ASCII":
                dc = DataContent(cfg=cfg, dat_text=self.dat_text)
            else:
                dc = DataContent(cfg=cfg, dat_bytes=self.sections.dat_bytes)
            return dc.data if dc else None
        except Exception as e:
