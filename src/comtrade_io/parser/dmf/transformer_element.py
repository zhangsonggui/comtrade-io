...
class TransformerWindingSection:
    ...
    @classmethod
    def from_xml(
        cls, element: Element, ns: dict, analog_channels: dict = None
    ) -> TransformerWinding:
        """
        从XML元素解析变压器绕组
        ...
        """
        location = TransWindLocation.from_value(
            element.get("location", ""), default=TransWindLocation.HIGH
        )
        ...
        wg = WindGroup(
            ...
            wind_flag=WindFlag.from_value(
                wg_elem.get("wgroup", "") if wg_elem else "", default=WindFlag.Y
            ),
        )
        ...
        acv_chn_elem = (
            element.find("scl:ACVChn", ns) if "scl" in ns else element.find("ACVChn")
        )
        if acv_chn_elem is not None:
            tfw.voltage = ACVBranch.from_xml(
        ...
        igap_elem = (
            element.find("scl:Igap", ns) if "scl" in ns else element.find("Igap")
        )
    ...
    def from_xml(
        ...
        if "scl" in ns:
            transformer.trans_winds = [
                TransformerWindingSection.from_xml(
                    tw, ns, analog_channels=analog_channels
                )
