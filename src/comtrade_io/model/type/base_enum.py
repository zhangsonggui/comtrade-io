...
class BaseEnum(Enum):
    ...
    def from_name(cls, name: str) -> T:
        ...
        异常:
            ValueError: 找不到对应名称的枚举成员时抛出
        """
        for member in cls:
            if member.name.upper() == name.upper():
                return cast(T, member)
        raise ValueError(
            f"无效的{cls.__name__}名称: {name}，可选名称: {[m.name for m in cls]}"
        )

    @classmethod
    def _match_value(cls, value: Any) -> T | None:
        """在枚举成员中查找匹配值的成员
