...
class TestDataShape:
    ...
    def test_dataframe_shape(self, comtrade_data, expected_df):
        ...
        assert (
            row_num == expected_df.shape[0]
        ), f"行数不匹配: {row_num} vs {expected_df.shape[0]}"
        expected_col_num = expected_df.shape[1] - 1
        assert (
            col_num == expected_col_num
        ), f"列数不匹配: {col_num} vs {expected_col_num}"
...
class TestDigitalColumns:
    ...
    def test_digital_all_values(self, comtrade_data, expected_df, comtrade_cfg):
        ...
        for col_idx in range(digital_count):
            ...
            assert np.array_equal(actual, expected), f"数字量通道{col_idx + 1}值不匹配"
...
class TestDataConsistency:
    ...
    def test_last_sample_index(self, comtrade_data):
        ...
        assert (
            last_idx == expected_count
        ), f"最后采样点索引应为{expected_count}, 实际为{last_idx}"
